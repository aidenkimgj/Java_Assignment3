...
public class ReservationsTab extends TabBase {
   ...
   class FindReserveListener implements ActionListener {
      ...
      public void actionPerformed(ActionEvent e) {
         ...
         // find the reservation record based on user's search.
         for (int i = 0; i < manager.getReservations().size(); i++) {
            ...
            if (!txtfieldName_Search.getText().equals("") && !txtfieldAirline_Search.getText().equals("")
               ...
               if (manager.getReservations().get(i).getName().equals(txtfieldName_Search.getText())
                     && manager.getReservations().get(i).getCode().equals(txtfieldCode_Search.getText())
                     && manager.getReservations().get(i).getFlight().getAirline()
                           .equals(txtfieldAirline_Search.getText())) {

                  reserveModel.addElement(manager.getReservations().get(i));
               }
   ...
   class MyListSelectionListener implements ListSelectionListener {
      ...
      public void valueChanged(ListSelectionEvent e) {
         ...
         if (!e.getValueIsAdjusting()) {
            ...
            if (selected > -1) {
               txtField_Code_Reserve.setText(reserveModel.get(selected).getCode());
               txtField_Flight_Reserve.setText(reserveModel.get(selected).getFlight().getCode());
               txtField_Airline_Reserve.setText(reserveModel.get(selected).getFlight().getAirline());
               txtField_Cost_Reserve.setText(
                     (String.format("%s%.2f", "$ ", reserveModel.get(selected).getFlight().getCostPerSeat())));
               txtField_Name_Reserve.setText(reserveModel.get(selected).getName());
               txtField_Citizen_Reserve.setText(reserveModel.get(selected).getCitizenship());
               if (reserveModel.get(selected).isActive()) {
   ...
   class UpdateListener implements ActionListener {
      ...
      public void actionPerformed(ActionEvent e) {
         ...
         for (int i = 0; i < manager.getReservations().size(); i++) {
            ...
            if (manager.getReservations().get(i).getCode().equals(txtField_Code_Reserve.getText())) {

               if (cboxStatus_Reserve.getSelectedItem().equals("Inactive")) {

                  manager.getReservations().get(i).setActive(false);

               } else {
                  manager.getReservations().get(i).setActive(true);
               }
               // in order to handle with exception
               try {

                  manager.getReservations().get(i).setCitizenship(txtField_Citizen_Reserve.getText());
               } catch (InvalidCitizenshipException e1) {
                  JOptionPane.showMessageDialog(null,"Invalid Citizenship!");
               }
               try {
                  manager.getReservations().get(i).setName(txtField_Name_Reserve.getText());
         ...
         //save data to the bin file using persist method.
         manager.persist();
      }
