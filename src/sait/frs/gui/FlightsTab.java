...
public class FlightsTab extends TabBase {
	 ...
	 */
	public FlightsTab(Manager manager) throws IOException {
		...
		// In order to load flight information and airport information.
//		FlightLoad();
//		AirportLoad();
	...
	/**
	 * This method read csv data and load to flight's ArrayList with flight type.
	 * 
	 * @throws IOException - if the code inside main method doesn't work, it throws
	 *                     IOException.
	 */
//	private void FlightLoad() throws IOException {
//		// Use Scanner to read file.
//		Scanner sc = new Scanner(new File(manager.FLIGHTS_TEXT));
//
//		String flightInfo = "";
//		String[] line;
//		while (sc.hasNext()) {
...
//			flight = new Flight(line[0], line[1], line[2], line[3], line[4], line[5], Integer.parseInt(line[6]),
//					Double.parseDouble(line[7]));
//			manager.getFlights().add(flight);
//		}
//		sc.close();
//	}

	/**
	 * This method read csv data and load to airport's ArrayList with airport type.
	 * 
	 * 
	 * @throws FileNotFoundException - if the code inside main method doesn't work it throws IOException.
	 */
//	private void AirportLoad() throws FileNotFoundException {
//		// Use Scanner to read file.
//		Scanner sc = new Scanner(new File(manager.AIRPORTS_TEXT));
//		String airportInfo = "";
//
//		while (sc.hasNext()) {
//			airportInfo = sc.nextLine();
//			manager.getAirports().add(airportInfo);
//		}
//		sc.close();
//	}
	...
	private JPanel createCenterPanel() {
		...
		comboBox_From = new JComboBox(manager.getAirports().toArray());
		...
		// set up JComboBox location and create comboBox list using getAirports() method
		// from manager and added to panel.
		comboBox_To = new JComboBox(manager.getAirports().toArray());
		...
		// JComboBox.
		String days[] = { manager.WEEKDAY_ANY, manager.WEEKDAY_SUNDAY, manager.WEEKDAY_MONDAY, manager.WEEKDAY_TUESDAY,
				manager.WEEKDAY_WEDNESDAY, manager.WEEKDAY_THURSDAY, manager.WEEKDAY_FRIDAY, manager.WEEKDAY_SATURDAY };
	...
	class FindBtnListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			for (Flight flight: manager.findFlights(from,to,day)) {
...
//			for (int i = 0; i < manager.getFlights().size(); i++) {
//
//				if (comboBox_Day.getSelectedItem().equals("Any")) {
//					if ((manager.getFlights().get(i).getFrom().equals(comboBox_From.getSelectedItem())
//							&& (manager.getFlights().get(i).getTo().equals(comboBox_To.getSelectedItem())))) {
//						flightsModel.addElement(manager.getFlights().get(i));
//
//					}
//
//				} else {
...
//					if ((manager.getFlights().get(i).getFrom().equals(comboBox_From.getSelectedItem()))
//							&& (manager.getFlights().get(i).getTo().equals(comboBox_To.getSelectedItem()))
//							&& (manager.getFlights().get(i).getWeekday().equals(comboBox_Day.getSelectedItem()))) {
//
//						flightsModel.addElement(manager.getFlights().get(i));
//
//					}
			...
			try {
				...
				JOptionPane.showMessageDialog(null,
						"Reseration created. Your code is "
								+ manager.makeReservation(manager.findFlightByCode(textField_Flight_Reserve.getText()),
				...
				JOptionPane.showMessageDialog(null,
						"Please find your flight");
			} catch (NoMoreSeatsException e1) {
				JOptionPane.showMessageDialog(null,
						"No more seat right now");
	...
	class MyListSelectionListener implements ListSelectionListener {
		...
		public void valueChanged(ListSelectionEvent e) {
			...
			if (!e.getValueIsAdjusting()) {
				...
				if (selected > -1) {
					// put flight's information which user want to reserve in the Textfield area's
					// with formatted.
					textField_Flight_Reserve.setText(flightsModel.get(selected).getCode());
					textField_Airline_Reserve.setText(flightsModel.get(selected).getAirline());
